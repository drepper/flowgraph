...
namespace {
  ...
  constexpr std::string_view trim(const std::string_view s) noexcept
    post(r : r.size() <= s.size())
  {
    constexpr std::string_view blank = " \t\r\n";
    const std::size_t a = s.find_first_not_of(blank);
    return a == std::string_view::npos ? std::string_view{}
           : s.substr(a, s.find_last_not_of(blank) + 1 - a);
  }
  ...
  struct name_table {
    ...
    //! \param s the name
    //! \return its number
    unsigned long of(std::string_view s)
      post(r : r != 0)
    {
...
#pragma GCC diagnostic ignored "-Wmismatched-new-delete"
  ...
  flowgraph::graph_source records(std::istream& in, std::string_view fname,
                                  name_table& names, std::vector<unsigned>& where)
  {
    unsigned lineno = 0;
    const auto fail = [&fname, &lineno](std::string_view msg) {
      throw std::runtime_error(std::format("{}:{}: {}", fname, lineno, msg));
    };
    ...
    for (std::string line; std::getline(in, line); ) {
      ...
      if (type == 'B' || type == 'N' || type == 'R') {
        ...
        if (std::from_chars(w.data(), w.data() + w.size(), weight).ptr
            != w.data() + w.size()) [[unlikely]]
          fail("expected node name and weight");
        // a node with nothing to say for itself is labelled with its name
        const std::string_view label = trim(rest);
        co_yield flowgraph::node_record{ names.of(id), label.empty() ? id : label, weight,
...
#pragma GCC diagnostic pop
  ...
  flowgraph::graph read_graph(std::istream& in, std::string_view fname, name_table& names)
  {
    ...
    try {
      ...
      const std::string msg =
        e.why == reason::no_nodes ? std::string("no nodes")
        : e.why == reason::no_begin ? std::string("no begin node")
        : e.why == reason::duplicate
          ? std::format("duplicate node name '{}'", names.name(e.node))
  ...
  {
    return std::ranges::fold_left(s, h, [](std::uint64_t acc, char c) {
      return (acc ^ static_cast<unsigned char>(c)) * 0x100000001b3ull;
    });
  }
  ...
  flowgraph::attributes hashed(const name_table& names, const flowgraph::draw_item& what)
  {
    const std::uint64_t h = std::visit(
      [&names](const auto& x) {
        ...
        if constexpr (std::same_as<std::remove_cvref_t<decltype(x)>, flowgraph::node_item>)
          ...
          return fold(x.backward ? "back" : "",
                      fold(names.name(x.to), fold("->", fold(names.name(x.from), seed))));
      },
      what);
    ...
    const std::array<std::array<unsigned, 3>, 6> ramp = { {
      { 255, up, 0 }, { down, 255, 0 }, { 0, 255, up },
      { 0, down, 255 }, { up, 0, 255 }, { 255, 0, down } } };
    ...
    a.colour = flowgraph::rgb{ chan(0), chan(1), chan(2) };
    if (std::holds_alternative<flowgraph::edge_item>(what)) {
      static constexpr std::array<flowgraph::line_style, 8> styles = {
        flowgraph::line_style::solid, flowgraph::line_style::solid,
        flowgraph::line_style::solid, flowgraph::line_style::solid,
        flowgraph::line_style::dense_dash, flowgraph::line_style::dash,
        flowgraph::line_style::sparse_dash, flowgraph::line_style::blink };
  ...
  constexpr std::string_view usage =
    ...
    "\n"
    "Other:\n"
    "  -p, --page           show the drawing one screen at a time; cursor keys\n"
  ...
  {
    switch (k) {
    case flowgraph::node_kind::begin: return "begin";
    case flowgraph::node_kind::ret:   return "return";
  ...
  {
    return pts | std::views::transform([](flowgraph::point p) {
                   return std::format(" ({},{})", p.row, p.col);
                 })
               | std::views::join
  ...
  void describe(const flowgraph::layout& l, const name_table& names, std::ostream& out)
  {
    ...
    std::ranges::for_each(l.nodes, [&](const flowgraph::layout_node& nd) {
      std::println(out, "  {} \"{}\" weight={} {} layer={} at row={} col={} size={}x{}",
                   names.name(nd.id), nd.label, nd.weight, kindname(nd.kind), nd.layer, nd.row, nd.col,
                   nd.height, nd.width);
    });
    std::println(out, "edges:");
    std::ranges::for_each(l.edges, [&](const flowgraph::layout_edge& e) {
      std::println(out, "  {} -> {} ({}):{}", names.name(l.nodes[e.from].id),
                   names.name(l.nodes[e.to].id),
                   e.backward ? "backward" : "forward", points(e.route.pts));
    });
    if (! l.marks.empty()) {
      std::println(out, "markers:");
      std::ranges::for_each(l.marks, [&out](const flowgraph::polyline& p) {
        std::println(out, " {}", points(p.pts));
      });
    }
  }
...
int main(int argc, char* argv[])
{
  ...
  const std::array<option, 10> opts = { {
    { "--row", 'r', &row, nullptr, LONG_MIN }, { "--col", 'c', &col, nullptr, LONG_MIN },
  ...
  try {
    ...
    const auto paint = [&names](const flowgraph::draw_item& what) {
      return hashed(names, what);
    };
    const flowgraph::painter how = want_plain ? flowgraph::painter(flowgraph::plain)
                                              : flowgraph::painter(paint);
    if (want_page) {
      ...
      flowgraph::write_xpm(l, int(row), h, int(col), w, unsigned(zoom), want_dark,
                           std::cout, how);
    else
