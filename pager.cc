...
namespace {
  ...
  std::pair<int, int> screen_size()
    post(r : r.first > 0 && r.second > 0)
  {
    ::winsize ws{};
    if (::ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) != 0 || ws.ws_row == 0
        || ws.ws_col == 0) [[unlikely]]
      return { 24, 80 };
  ...
  std::string base64(const std::vector<unsigned char>& d)
    post(r : r.size() == (d.size() + 2) / 3 * 4)
  {
    static constexpr std::string_view tbl = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                            "abcdefghijklmnopqrstuvwxyz0123456789+/";
    return d | std::views::chunk(3)
             | std::views::transform([](auto three) {
                 const std::size_t k = std::size_t(std::ranges::distance(three));
                 const unsigned n = unsigned(three[0]) << 16
                                    | (k > 1 ? unsigned(three[1]) << 8 : 0u)
                                    | (k > 2 ? unsigned(three[2]) : 0u);
                 return std::array{ tbl[n >> 18 & 63], tbl[n >> 12 & 63],
                                    k > 1 ? tbl[n >> 6 & 63] : '=',
                                    k > 2 ? tbl[n & 63] : '=' };
               })
             | std::views::join
  ...
  std::vector<unsigned char> pixels(const flowgraph::layout& l, const unsigned mag,
                                    int top, int left, int rows, int cols,
                                    flowgraph::painter how)
    pre(mag > 0)
    post(r : r.size() == std::size_t(l.rows) * std::size_t(l.cols) * mag * mag * 4)
  {
    const std::vector<flowgraph::cell_paint> kd =
      flowgraph::classify(l, 0, l.rows, 0, l.cols, how);
    const auto colour = [&](int r, int c) {
      ...
      return pixel{ static_cast<unsigned char>(k.colour->red >> 8),
                    static_cast<unsigned char>(k.colour->green >> 8),
                    static_cast<unsigned char>(k.colour->blue >> 8), 0xff };
    };
    // every cell mag pixels wide, every row of cells mag rows of pixels high
    return std::views::iota(0, l.rows)
           | std::views::transform([&](int r) {
               const std::vector<unsigned char> row =
                 std::views::iota(0, l.cols)
                 | std::views::transform([&, r](int c) {
                     return std::views::repeat(colour(r, c), mag) | std::views::join;
                   })
                 | std::views::join
                 | std::ranges::to<std::vector>();
               return std::views::repeat(row, mag) | std::views::join
                      | std::ranges::to<std::vector>();
             })
           | std::views::join
           | std::ranges::to<std::vector>();
  ...
  std::string transmit(const flowgraph::layout& l, unsigned mag,
                       int top, int left, int rows, int cols, flowgraph::painter how)
    pre(mag > 0)
  {
    ...
    return b64 | std::views::chunk(piece) | std::views::enumerate
               | std::views::transform([&](auto ic) {
                   const auto [i, chunk] = ic;
                   return std::format("\033_G{}m={};{}\033\\",
                                      i == 0 ? std::format("a=t,i=1,f=32,s={},v={},q=2,",
                                                           std::size_t(l.cols) * mag,
                                                           std::size_t(l.rows) * mag)
                                             : std::string(),
                                      std::size_t(i) + 1 == pieces ? 0 : 1,
  ...
  {
    const std::vector<std::optional<int>> nums =
      rep | std::views::split(';')
          | std::views::transform([](auto part) -> std::optional<int> {
  ...
  struct terminal {
    ...
    ~terminal()
    {
      if (! ok)
        return;
      std::print("\033_Ga=d,d=A,q=2\033\\"
...
bool page(const flowgraph::layout& l, flowgraph::painter how)
{
  ...
  for (;;) {
    ...
    ::winsize ws{};
    const bool px_known = image && ::ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == 0
                          && ws.ws_xpixel > 0 && ws.ws_ypixel > 0;
    const int cw = px_known ? ws.ws_xpixel / ws.ws_col : 0;
    const int ch = px_known ? ws.ws_ypixel / ws.ws_row : 0;
    const auto fits = [&](unsigned m) {
      return l.cols * int(m) <= scols / 2 * cw && l.rows * int(m) <= vh * ch;
    };
    ...
    if (show != 0) {
      ...
      // the drawing changed.
      if (show != sent || top != sent_top || left != sent_left
          || vh != sent_rows || vw != sent_cols) {
      ...
      frame += std::format("\033_Ga=d,d=i,i=1,q=2\033\\\033[1;{}H"
                           "\033_Ga=p,i=1,C=1,q=2\033\\",
                           scols - icols + 1);
    }

    std::string status = std::format("{},{} - {},{}  of {}x{}{}   arrows scroll"
                                     "  q quits",
                                     top, left,
                                     std::min(top + vh, l.rows) - 1,
                                     std::min(left + vw, l.cols) - 1,
                                     l.rows, l.cols,
                                     show != 0 ? std::format("  x{}", show) : "");
    ...
    if (button == 0 && show != 0 && cw > 0 && ch > 0) {
      ...
      if (ix >= 0 && ix < icols && iy >= 0 && gr >= top && gr < top + vh
          && gc >= left && gc < left + vw) {
        dragging = true;
        grab_x = mx;
        grab_y = my;
